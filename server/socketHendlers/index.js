    ...
    messageNamespace.on('connection', (socket) => {
        socket.on('get_chat_list', async () => {
            ...
            const dataList = list.map(async chat => {
                ...
                const otherUser = chat.users.find(user => user._id.toString() !== socket.user._id.toString());
        ...
        socket.on('joinRoom', async (data) => {
            try {
                ...
                const conversation = await Conversation.findById(data).populate({
                    path: 'messages',
                    populate: {
                        path: 'sender',
                        model: 'users',
                ...
                const otherUser = conversation.users.find(user => user._id.toString() !== socket.user._id.toString());
                ...
                socket.emit('get_conversation', {
                    ...
                    user: {
                        ...
                        avatar: otherUser.avatar ? otherUser.avatar : otherUser.profileImages[0].url,
        ...
        socket.on('send_message', async (data) => {
            try {
                ...
                socket.to(userSocketMap.get(data.otherUser_id.toString())).emit('receive_message', {
