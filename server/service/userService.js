...
module.exports = {
  ...
  loginUser: async (emailOrUsername, password) => {
    ...
    if (!matchedPassword) {
      throw new CustomError(
        "The password for this user is not correct!",
        "IncorrectPassword"
      );
    }
  ...
  registerUser: async (
    ...
    const userInstance = new UserModel({
      ...
      phone: phoneNumber,
      profileImages: imagesResults
        ? imagesResults.filter((image) => image).map((image) => image._id)
        : undefined,
    });
  ...
  getUserGeneralSettingsData: async (sessionID) => {
    // Validate input
    if (!sessionID) {
      throw new CustomError("No session ID provided!", "NoSessionID");
    }
    ...
    // Fetch user data
    const userData = await UserModel.findById({ _id: sessionID })
      .populate("avatar")
      .populate("profileImages")
      .populate("address");
