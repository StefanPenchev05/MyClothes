...
function StepperContent() {
  ...
  const { setEmail, setSelectedDate, setFirstName, setLastName, setGender } =
    useTheme();
  const {
    email,
    password,
  ...
  const { gender, country, city, address, phone, images, activeStep, steps } =
    useTheme();
    ...
    switch (activeStep) {
      case 0: // Account Setup
        const validEmail = validateField(
          ...
          setEmail,
          validateEmail,
          t("registration.Email is required"),
          t("registration.Invalid email address")
        );
        const validUser = validateField(
          username,
          setUsername,
          validUsername,
          t("registration.Username is required"),
          t(
            "registration.Username must be and between 4 and 15 characters long"
          )
        );
        const validPassword = validateField(
          ...
          setPassword,
          validatePassword,
          t("registration.Password is required"),
          t("registration.Password must be at least 8 characters long")
        );

        const validConfirmPassword = validateField(
          confirmPassword,
          setConfirmPassword,
          (confirmPassword: string) => password.value === confirmPassword,
          t("registration.Confirm Password is required"),
          t("registration.Passwords do not match")
        );
        isValid =
          validEmail && validUser && validPassword && validConfirmPassword;
        break;
      case 1: // Personal Details
        const validFirstName = validateField(
          ...
          setFirstName,
          validateFirstName,
          t("registration.First name is required"),
          t("registration.Invalid First name")
        );
        const validLastName = validateField(
          lastName,
          setLastName,
          validateLastName,
          t("registration.Last name is required"),
          t("registration.Invalid Last name")
        );
        const validGender = validateField(
          gender,
          setGender,
          (value: string) => value !== "",
          t("registration.Gender is required"),
          ""
        );
        //@ts-ignore
        ...
        isValid =
          validFirstName && validLastName && validGender && validDateOfBirth;
        break;
      case 2:
        isValid = true;
        break;
  ...
  const onBack = () => {
    setSteps((prevSteps) => {
      ...
      newSteps[activeStep - 1].icon = React.cloneElement(
        newSteps[activeStep - 1].icon,
  ...
  const onFinish = async () => {
    ...
    const response = await sendData("/user/registration", {
      data: completedData,
    });
          ...
          >
            <Button
              ...
              style={{
                ...
                backgroundColor: activeStep > 0 ? "#696cff" : "#c4c4c4",
              }}
              className="font-medium rounded-lg h-12 text-base w-1/2"
              onClick={onBack}
